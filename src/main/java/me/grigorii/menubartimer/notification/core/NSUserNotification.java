...
import me.grigorii.menubartimer.notification.libraries.NSUserNotificationCInterface;
...
public class NSUserNotification {
	...
	public NSUserNotification() {
		if (library == null) {
            if (getClass().getResource("/") == null) {
                ...
                System.setProperty("jna.library.path", getClass().getResource("/").toString());
            }
	...
	public void setSoundName(String name) {
		...
		byte[] inputBytes = bytesFromString(name + '\000');
		Pointer informativeText = new Memory(inputBytes.length);
		informativeText.write(0L, inputBytes, 0, inputBytes.length);
		library.NSUserNotificationSetSoundName(this.notification, informativeText);
	}
